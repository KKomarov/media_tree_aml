...
avl_error_code_t avl68x2_demod_get_ssi(uint16_t *pusSSI, avl68x2_chip *chip)
{
	...
	if (chip->chip_pub->cur_demod_mode == AVL_DVBSX)
		agc_polarity = chip->chip_pub->dvbsx_config.eDVBSxAGCPola;
	else if (chip->chip_pub->cur_demod_mode == AVL_DVBTX)
		agc_polarity = chip->chip_pub->dvbtx_config.eDVBTxAGCPola;
	else if (chip->chip_pub->cur_demod_mode == AVL_DVBC)
		agc_polarity = chip->chip_pub->dvbc_config.eDVBCAGCPola;
	else if (chip->chip_pub->cur_demod_mode == AVL_ISDBT)
		agc_polarity = chip->chip_pub->isdbt_config.eISDBTAGCPola;

	*pusSSI = 0;
	r = avl_bms_read16(chip->chip_pub->i2c_addr,
			   stBaseAddrSet.fw_status_reg_base + rs_rf_agc_saddr_offset, 
			   pusSSI);
