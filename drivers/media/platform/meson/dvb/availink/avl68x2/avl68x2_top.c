...
static char stdout_tmp_buf[4096];
...
{
	ssize_t s_len;
	s_len = read_stdout(len, &global_priv->frontend);
	unsigned long uncopied = copy_to_user(buffer, stdout_buf, s_len);
	return (s_len - uncopied);
}
			...
			struct dtv_frontend_properties *props)
{
	...
	uint16_t ssi = 0;
	uint16_t sqi = 0;
	uint16_t rf_strength = 0;
	int32_t ssi32 = 0;
	int32_t ssi_relative = 0;
	const int32_t ssi_max = 58982;
	const int32_t ssi_min = 29790;
	int8_t demod_id =
	...
	{
		ret = avl68x2_demod_i2c_passthru_on(priv->chip);
		ret |= fe->ops.tuner_ops.get_rf_strength(fe, &rf_strength);
		//printk("get_rf_strength %d\n", rf_strength);
		ret = avl68x2_demod_i2c_passthru_off(priv->chip);
	}
	...
	// IFAGC has a gain range of ~58dB
	ssi32 = (((int32_t)ssi - ssi_min) * 58000) / (ssi_max - ssi_min);
	ssi32 = (ssi32 > 46000) ? 46000 : ssi32;
	ssi32 = (ssi32 < 0) ? 0 : ssi32;

	ssi_relative = (8 * (ssi32 + rf_strength)) / 10;
	ssi_relative = (ssi_relative > 65535) ? 65535 : ssi_relative;

	props->strength.len = 2;
	props->strength.stat[0].scale = FE_SCALE_DECIBEL;
	props->strength.stat[0].svalue = -96000 + ssi32 + rf_strength;
	props->strength.stat[1].scale = FE_SCALE_RELATIVE;
	props->strength.stat[1].uvalue = (uint16_t)ssi_relative;

	ret = avl68x2_demod_get_lock_status(&lock, priv->chip);
	if(lock)
	{

		ret |= avl68x2_demod_get_snr(&SNR_x100db, priv->chip); //100ths
		props->cnr.len = 2;
		props->cnr.stat[0].scale = FE_SCALE_DECIBEL;
		props->cnr.stat[0].svalue = SNR_x100db * 10; //svalue = 1000ths

		ret |= avl68x2_demod_get_sqi(&sqi, priv->chip); //[0,100]
		props->cnr.stat[1].scale = FE_SCALE_RELATIVE;
		props->cnr.stat[1].uvalue = ((uint32_t)sqi * 0xFFFF) / 100; //uvalue = norm to 65535
